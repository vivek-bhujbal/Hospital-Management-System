...
import { Clock3, Eye, Loader2, ShieldCheck, UserRound } from 'lucide-react'

import {
  createStaffAccountAction,
  getStaffAccountAction,
  setHospitalManagerActiveAction,
  updateReceptionistPageAccessAction,
...
export default function StaffAccountsPanel({ accounts }: { accounts: AccountSummary[] }) {
  ...
  async function updatePageAccess(formData: FormData) {
    setAccessError('')
    setAccessSuccess('')
    const result = await updateReceptionistPageAccessAction(formData)
    if (result.error) return setAccessError(result.error)
    if (result.permissions) {
      setDetails((current) => current?.profile?.type === 'receptionist'
        ? {
            ...current,
            profile: { ...current.profile, permissions: result.permissions || null },
          }
        : current)
    }
    setAccessSuccess('Page access updated successfully.')
  }

  async function updateShift(formData: FormData) {
    setShiftError('')
    setShiftSuccess('')
    const result = await updateStaffShiftAction(formData)
    if (result.error || !result.account) return setShiftError(result.error || 'Failed to update staff shift.')
    setDetails(result.account)
    setShiftSuccess('Working shift updated successfully.')
  }
    ...
    <div className="space-y-6">
      <section className="rounded-xl border bg-white p-6 shadow-sm">
        ...
        <form ref={formRef} action={submit} className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <input name="contact" aria-label="Staff contact number" maxLength={20} placeholder="Contact" className="rounded-lg border p-3" />
      ...
      <StaffDetailsModal details={details} selectedRole={selectedRole} setSelectedRole={setSelectedRole} changeRole={changeRole} roleError={roleError} updatePageAccess={updatePageAccess} accessError={accessError} accessSuccess={accessSuccess} updateShift={updateShift} shiftError={shiftError} shiftSuccess={shiftSuccess} onClose={() => setDetails(null)} />
    </div>
  )
}

function StaffDetailsModal({ details, selectedRole, setSelectedRole, changeRole, roleError, updatePageAccess, accessError, accessSuccess, updateShift, shiftError, shiftSuccess, onClose }: {
  details: StaffAccountDetails | null
  selectedRole: string
  setSelectedRole: (role: string) => void
  changeRole: (formData: FormData) => Promise<void>
  roleError: string
  updatePageAccess: (formData: FormData) => Promise<void>
  accessError: string
  accessSuccess: string
  updateShift: (formData: FormData) => Promise<void>
  shiftError: string
  shiftSuccess: string
  onClose: () => void
}) {
    ...
    {details && <div className="space-y-6">
      ...
      {profile ? <section>
        ...
        <dl className="mt-3 grid gap-x-6 gap-y-4 rounded-xl border border-slate-200 p-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {profile.type === 'doctor' ? <><Detail label="Specialization" value={profile.specialization || 'Not provided'} /><Detail label="Department ID" value={profile.department_id ? `#${profile.department_id}` : 'Not assigned'} /><Detail label="Consultation fee" value={profile.consultation_fee != null ? String(profile.consultation_fee) : 'Not provided'} /><Detail label="Contact" value={profile.contact || 'Not provided'} /><Detail label="Working hours" value={profile.timing_start && profile.timing_end ? `${profile.timing_start} – ${profile.timing_end}` : 'Not provided'} /></> : <><Detail label="Designation" value={profile.designation || 'Not provided'} /><Detail label="Contact" value={profile.contact || 'Not provided'} /><Detail label="Joining date" value={profile.joining_date || 'Not provided'} /><Detail label="Shift" value={profile.shift_start && profile.shift_end ? `${profile.shift_start} – ${profile.shift_end}` : 'Not provided'} /></>}
        ...
        {profile.type === 'receptionist' && <form action={updatePageAccess} className="mt-3 rounded-xl border border-slate-200 p-4">
          <input type="hidden" name="employee_id" value={profile.id} />
          <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Assigned page access</p><p className="mt-1 text-sm text-slate-600">Choose which receptionist pages this account can use.</p></div>
          {accessError && <p role="alert" className="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{accessError}</p>}
          {accessSuccess && <p role="status" className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">{accessSuccess}</p>}
          <div className="mt-4 grid gap-3 sm:grid-cols-2">{RECEPTIONIST_PAGE_ACCESS.map(([name, label, description]) => (
            <label key={name} className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 transition hover:border-brand-300 hover:bg-brand-50/40">
              <input type="checkbox" name={name} defaultChecked={Boolean(profile.permissions?.[name])} className="mt-0.5 h-4 w-4 rounded border-slate-300 text-brand-700 focus:ring-brand-600" />
              <span><span className="block text-sm font-semibold text-slate-900">{label}</span><span className="mt-1 block text-xs leading-5 text-slate-500">{description}</span></span>
            </label>
          ))}</div>
          <div className="mt-4 flex justify-end"><SubmitButton>Save page access</SubmitButton></div>
        </form>}
      ...
      <form action={updateShift} className="rounded-xl border border-sky-200 bg-sky-50/50 p-4">
        <div className="flex items-start gap-3"><Clock3 className="mt-0.5 h-5 w-5 text-sky-700" /><div><h3 className="font-semibold text-slate-900">Working shift</h3><p className="mt-1 text-sm text-slate-600">Set this staff member&apos;s overall start and end time.</p></div></div>
        {shiftError && <p role="alert" className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{shiftError}</p>}
        {shiftSuccess && <p role="status" className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">{shiftSuccess}</p>}
        <input type="hidden" name="id" value={details.id} />
        <div className="mt-4 max-w-md"><TimeFields prefix="shift" start={profile?.type === 'doctor' ? profile.timing_start : profile?.shift_start} end={profile?.type === 'doctor' ? profile.timing_end : profile?.shift_end} required /></div>
        <div className="mt-4 flex justify-end"><SubmitButton>Save shift</SubmitButton></div>
      </form>
...
function TimeFields({ prefix, start, end, required = false }: { prefix: 'timing' | 'shift', start?: string | null, end?: string | null, required?: boolean }) {
  return <div className="grid grid-cols-2 gap-2"><label><span className="mb-1.5 block text-sm font-semibold text-slate-700">Start time</span><input name={`${prefix}_start`} type="time" required={required} defaultValue={start?.slice(0, 5) || ''} className="hms-input" /></label><label><span className="mb-1.5 block text-sm font-semibold text-slate-700">End time</span><input name={`${prefix}_end`} type="time" required={required} defaultValue={end?.slice(0, 5) || ''} className="hms-input" /></label></div>
}
