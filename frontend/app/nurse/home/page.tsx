...
export default async function NurseDashboard() {
  ...
  const metrics = [
    ...
    ['Require nursing tasks', dashboard.patients_requiring_tasks],
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {dashboard.urgent_alerts.length === 0 ? <p className="mt-5 rounded-xl bg-emerald-50 p-5 text-sm font-medium text-emerald-800">No high-priority or emergency tasks assigned.</p> : <div className="mt-4 space-y-3">{dashboard.urgent_alerts.map((alert) => <Link key={alert.task_id} href={`/nurse/patient/${alert.patient_id}`} className="block rounded-xl border border-slate-200 p-4 hover:border-blue-300"><div className="flex justify-between gap-3"><p className="font-semibold text-slate-900">{alert.patient_name}</p><span className={`rounded-full px-2 py-0.5 text-xs font-semibold capitalize ring-1 ring-inset ${priorityClass(alert.priority)}`}>{alert.priority}</span></div><p className="mt-2 text-sm text-slate-700">{alert.description}</p><p className="mt-2 text-xs text-slate-500">Due {alert.due_at ? new Date(alert.due_at).toLocaleString() : 'not specified'}</p></Link>)}</div>}
        </section>
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
