  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <SectionCard title="Recent prescriptions" action={<Link href="/pharmacist/prescriptions" className="text-sm font-semibold text-brand-700 dark:text-brand-300">View all</Link>}>
        {data.recent_prescriptions.length === 0 ? <EmptyState title="No pharmacy activity yet" description="New Doctor prescriptions will appear here for pharmacy review." /> : <div className="divide-y">{data.recent_prescriptions.map((item) => <Link key={item.id} href={`/pharmacist/prescriptions/${item.id}`} className="flex flex-col gap-3 p-5 hover:bg-brand-50/50 dark:hover:bg-brand-950/20 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-slate-900 dark:text-slate-100">{item.patient_name}</p><p className="mt-1 text-sm text-slate-500">{item.medicine || 'Medicine not specified'} · Dr. {item.doctor_name}</p></div><StatusBadge status={item.pharmacy_status} /></Link>)}</div>}
      </SectionCard>
